...
(() => {
    ...
    const wrapRangeWithChain = (range, chain) => {
        if (!chain.length) return;
        const contents = range.extractContents();
        let node = contents;
        chain.forEach((builder) => {
    ...
    const applyCommand = async (command, value = null, options = {}) => {
        const { skipSnapshot = false, rangeOverride = null } = options;
        ...
        const range = rangeOverride || getSelectionRange();
        if (!range) {
            return;
        }
    ...
    const applyCommands = async (commands) => {
        ...
        for (const { command, value } of commands) {
            const builder = inlineBuilders[command];
            if (!builder) {
                builders.length = 0;
                break;
            }
            builders.push(() => builder(value));
        }

        saveSnapshot();

        if (builders.length) {
            wrapRangeWithChain(range, builders);
        } else {
            for (const { command, value } of commands) {
                await applyCommand(command, value, { skipSnapshot: true, rangeOverride: range });
            }
        }
...
def exec_commands(web_instance, commands):
    ensure_helper(web_instance)
    serialized = json.dumps([{ "command": cmd, "value": val } for cmd, val in commands])
    web_instance.eval(f"window.formatAndEditHelper.applyCommands({serialized});")
...
def build_selection_menu(web_instance, menu, config):
    if config["enabled"]:
        ...
        # Combined actions
        combined_menu = selection_menu.addMenu("Combined Actions")
        for action_name, combo in combined_actions:
            action = QAction(action_name, menu)
            action.triggered.connect(lambda _, w=web_instance, c=combo: exec_commands(w, c))
...
combined_actions = [
    ("Bold + Underline", [("bold", None), ("underline", None)]),
    ("Bold + Red", [("bold", None), ("foreColor", "red")]),
    ("Bold + Underline + Red", [("bold", None), ("underline", None), ("foreColor", "red")]),
    ("Underline + Yellow Highlight", [("underline", None), ("backColor", "yellow")]),
]
